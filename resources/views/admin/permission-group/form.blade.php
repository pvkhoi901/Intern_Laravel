...
<form class="container-fluid" method="post" action="{{ route('permission-group.store') }}">
  ...
  <div class="row">
    <div class="d-flex justify-content-between">
      <h3> {{ __('permission_group.Create permission group:')}} </h3>
      @else
      <form class="container-fluid" method="post" action="{{ route('permission-group.update', $permissionGroup->id) }}">
        ...
        <div class="row">
          <div class="d-flex justify-content-between">
            <h3>{{ __('permission_group.Edit permission group:')}}  </h3>
            @endif
        ...
        <table class="table">
          <thead>
            ...
            <tr>
              ...
              <th scope="col">{{ __('permission_group.Created_at')}}</th>
              <th scope="col">{{ __('permission_group.Updated_at')}}</th>
            </tr>
          </thead>
          <tbody>
            <tr>
        ...
        <div class="container-fluid">
          <label for="name" class="form-label">{{ __('permission_group.Name')}}  </label>
